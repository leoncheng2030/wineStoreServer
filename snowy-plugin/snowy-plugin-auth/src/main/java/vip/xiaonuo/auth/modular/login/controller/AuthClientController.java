...
public class AuthClientController {
    ...
    @SaClientCheckLogin
    @PostMapping("/auth/c/uploadAvatar")
    public CommonResult<String> uploadAvatar(@RequestPart("file") MultipartFile file) {
        String s = devFileApi.uploadDynamicReturnUrl(file);
    ...
    public CommonResult<String> doLoginById(@RequestBody @Valid MiniLoginParam miniLoginParam) {
        return CommonResult.data(authService.doMiniProgramLogin(miniLoginParam, SaClientTypeEnum.C.getValue()));
    }

    @ApiOperationSupport(order = 11)
    @Operation(summary = "判断用户是否存在")
    @PostMapping("/auth/c/login")
    public CommonResult<String> login(@RequestBody @Valid MiniIsRegParam miniIsRegParam) {
        return CommonResult.data(authService.isLogin(miniIsRegParam,SaClientTypeEnum.C.getValue()));
    }
}
