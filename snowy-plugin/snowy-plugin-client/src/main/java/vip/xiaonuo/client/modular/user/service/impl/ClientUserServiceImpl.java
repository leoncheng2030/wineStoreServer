...
public class ClientUserServiceImpl extends ServiceImpl<ClientUserMapper, ClientUser> implements ClientUserService {
    ...
    @Resource
    private ClientUserAccountApi clientUserAccountApi;
    ...
    public Page<ClientUser> page(ClientUserPageParam clientUserPageParam) {
        ...
        if(ObjectUtil.isNotEmpty(clientUserPageParam.getPhone())) {
            queryWrapper.lambda().eq(ClientUser::getPhone, CommonCryptogramUtil.doSm4CbcEncrypt(clientUserPageParam.getPhone()));
        }
        ...
        if(ObjectUtil.isNotEmpty(clientUserPageParam.getIsAgent())) {
            queryWrapper.lambda().eq(ClientUser::getIsAgent, clientUserPageParam.getIsAgent());
        }
    ...
    public void add(ClientUserAddParam clientUserAddParam, String sourceFromType) {
        ...
        // 设置非代理
        clientUser.setIsAgent("NO");
        // 保存用户
        this.save(clientUser);
        // 插入扩展信息
        clientUserExtService.createExtInfo(clientUser.getId(), sourceFromType);
        // 插入用户账户信息
        clientUserAccountApi.createAccountInfo(clientUser.getId(),clientUser.getNickname(),clientUser.getPhone());
    }
    ...
    public void updateUserInfo(ClientUserUpdateInfoParam clientUserUpdateInfoParam) {
        String id = StpClientUtil.getLoginIdAsString();
        ClientUser clientUser = this.queryEntity(id);
        
        // 增强日志：记录更新前的用户信息
        log.info("=== 用户信息更新开始 ===");
        log.info("用户ID: {}", id);
        log.info("更新前用户信息: 昵称=[{}], 头像=[{}]", 
                clientUser.getNickname(), 
                clientUser.getAvatar() != null ? clientUser.getAvatar().substring(0, Math.min(50, clientUser.getAvatar().length())) + "..." : "null");
        log.info("请求参数: 昵称=[{}], 头像=[{}], 姓名=[{}], 性别=[{}], 生日=[{}]", 
                clientUserUpdateInfoParam.getNickname(),
                clientUserUpdateInfoParam.getAvatar() != null ? clientUserUpdateInfoParam.getAvatar().substring(0, Math.min(50, clientUserUpdateInfoParam.getAvatar().length())) + "..." : "null",
                clientUserUpdateInfoParam.getName(),
                clientUserUpdateInfoParam.getGender(),
                clientUserUpdateInfoParam.getBirthday());
        ...
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getName())) {
            log.info("更新姓名: {} -> {}", clientUser.getName(), clientUserUpdateInfoParam.getName());
            lambdaUpdateWrapper.set(ClientUser::getName, clientUserUpdateInfoParam.getName());
            hasUpdateFields = true;
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getAvatar())) {
            log.info("更新头像: [{}] -> [{}]", 
                    clientUser.getAvatar() != null ? "已有头像" : "无头像",
                    "新头像");
            lambdaUpdateWrapper.set(ClientUser::getAvatar, clientUserUpdateInfoParam.getAvatar());
            hasUpdateFields = true;
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getNickname()) && !clientUserUpdateInfoParam.getNickname().trim().isEmpty()) {
            log.info("更新昵称: [{}] -> [{}]", clientUser.getNickname(), clientUserUpdateInfoParam.getNickname());
            lambdaUpdateWrapper.set(ClientUser::getNickname, clientUserUpdateInfoParam.getNickname().trim());
            hasUpdateFields = true;
        } else {
            log.warn("昵称参数为空、null或只包含空格: [{}]", clientUserUpdateInfoParam.getNickname());
        }
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getGender())) {
            log.info("更新性别: {} -> {}", clientUser.getGender(), clientUserUpdateInfoParam.getGender());
        ...
        if(ObjectUtil.isNotEmpty(clientUserUpdateInfoParam.getBirthday())) {
            log.info("更新生日: {} -> {}", clientUser.getBirthday(), clientUserUpdateInfoParam.getBirthday());
        ...
        // 如果没有需要更新的字段，抛出异常
        if (!hasUpdateFields) {
            log.error("没有提供需要更新的用户信息字段，请求参数可能全部为空");
            throw new CommonException("没有提供需要更新的用户信息字段");
        }
        
        log.info("准备执行数据库更新，hasUpdateFields: {}", hasUpdateFields);
        
        // 更新指定字段
        boolean updateResult = this.update(lambdaUpdateWrapper);
        log.info("数据库更新结果: {}", updateResult);
        
        // 验证更新后的结果
        ClientUser updatedUser = this.queryEntity(id);
        log.info("更新后用户信息: 昵称=[{}], 头像=[{}]", 
                updatedUser.getNickname(), 
                updatedUser.getAvatar() != null ? updatedUser.getAvatar().substring(0, Math.min(50, updatedUser.getAvatar().length())) + "..." : "null");
        
        // 【关键修复】更新Token Session中的用户信息缓存
        try {
            // 将更新后的用户信息转换为Session对象
            SaBaseClientLoginUser updatedLoginUser = BeanUtil.copyProperties(updatedUser, SaBaseClientLoginUser.class);
            // 更新Token Session
            StpClientUtil.getTokenSession().set("loginUser", updatedLoginUser);
            log.info("用户{}个人信息更新成功，同时更新了Session缓存", id);
            log.info("=== 用户信息更新完成 ===");
        } catch (Exception e) {
            log.error("更新用户Session缓存失败，但数据库更新成功", e);
            // 这里不抛异常，因为数据库更新已经成功
        }
    }
    ...
    @Override
    public List<ClientUser> listByIds(ClientUserIdListParam idList) {
        if (ObjectUtil.isEmpty(idList.getIdList())) {
            return CollectionUtil.newArrayList();
        }
        LambdaQueryWrapper<ClientUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        // 只查询部分字段
        lambdaQueryWrapper.select(ClientUser::getId, ClientUser::getAvatar, ClientUser::getAccount, ClientUser::getName,
                        ClientUser::getSortCode)
                .in(ClientUser::getId, idList.getIdList()).orderByAsc(ClientUser::getSortCode);
    ...
    @Override
    public ClientLoginUser getByOpenid(String openid) {
        ClientUser clientUser = this.getOne(new LambdaQueryWrapper<ClientUser>().eq(ClientUser::getOpenid, openid));
        if(ObjectUtil.isNotEmpty(clientUser)) {
            transService.transOne(clientUser);
            ClientLoginUser clientLoginUser = BeanUtil.copyProperties(clientUser, ClientLoginUser.class);
            return clientLoginUser;
    ...
    @Override
    public ClientUser createUserWithOpenId(String account, String nickName, String avatarUrl, String openid, String unionid, String phoneNumber) {
        ClientUserAddParam clientUserAddParam = new ClientUserAddParam();
        clientUserAddParam.setAccount(account); // 使用处理后的账号
        clientUserAddParam.setName(nickName);
        clientUserAddParam.setNickname(nickName);
        clientUserAddParam.setAvatar(avatarUrl);
        clientUserAddParam.setPhone(phoneNumber);
        clientUserAddParam.setOpenid(openid);
        clientUserAddParam.setUnionid(unionid);
        
        log.info("创建微信用户: account={}, openid={}, phone={}", account, openid, phoneNumber);
        this.add(clientUserAddParam,"miniProgram");

        return this.getOne(new LambdaQueryWrapper<ClientUser>().eq(ClientUser::getOpenid, openid));
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void setUserAsAgent(String userId) {
        ClientUser clientUser = queryEntity(userId);
        clientUser.setIsAgent("YES");
        updateById(clientUser);
        
        // 通过API创建代理商记录
        wineAgentApi.createAgentRecord(userId);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void cancelUserAgent(String userId) {
        ClientUser clientUser = queryEntity(userId);
        clientUser.setIsAgent("NO");
        updateById(clientUser);
        
        // 通过API删除代理商记录
        wineAgentApi.deleteAgentRecord(userId);
    }

    @Override
    public List<ClientUser> listByIds(String[] userIds) {
        return listByIds(Arrays.asList(userIds));
    }
