...
public class ClientUserController {
     ...
     * @author [Your Name]
     * @date [Current Date]
     * @description 此接口用于前端用户选择器的分页查询，参考page方法实现。
     * 改动原因：匹配前端clientUserSelector调用，提供专用的selector分页接口。
     */
    @ApiOperationSupport(order = 6)
    @Operation(summary = "获取用户选择器分页")
     ...
     */
    @Operation(summary = "获取用户选择器分页")
    @PostMapping("/client/user/listByIds")
    public CommonResult<List<ClientUser>> listByIds(@RequestBody @Valid ClientUserIdListParam clientUserIdListParam) {
        return CommonResult.data(clientUserService.listByIds(clientUserIdListParam));
    }
    /**
     * 设置代理身份
     *
    ...
    @Operation(summary = "设置代理身份")
    @CommonLog("设置代理身份")
    @PostMapping("/client/user/setUserAsAgent")
    public CommonResult<String> setUserAsAgent(@RequestBody @Valid ClientUserIdParam clientUserIdParam) {
        clientUserService.setUserAsAgent(clientUserIdParam.getId());
        return CommonResult.ok();
    }
    ...
    @Operation(summary = "取消代理身份")
    @CommonLog("取消代理身份")
    @PostMapping("/client/user/cancelUserAgent")
    public CommonResult<String> cancelUserAgent(@RequestBody @Valid ClientUserIdParam clientUserIdParam) {
        clientUserService.cancelUserAgent(clientUserIdParam.getId());
        return CommonResult.ok();
    }
}
