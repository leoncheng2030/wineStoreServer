...
public class DevFileController {
     ...
     **/
    @PostMapping("/dev/file/uploadDynamicReturnPath")
    @Operation(summary = "上传本地文件返回url")
    @CommonLog("上传本地文件返回url")
    public CommonResult<String> uploadLocalReturnPath(@RequestPart("file") MultipartFile file){
        return CommonResult.data(devFileService.uploadReturnPath(DevFileEngineTypeEnum.LOCAL.getValue(), file,false));
    }
