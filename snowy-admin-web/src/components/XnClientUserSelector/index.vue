...
<script setup name="clientUserSelector">
	// 注意：此组件是参考XnUserSelector创建的客户端用户选择器
	// 修改原因：为客户端用户提供类似的用户选择界面和功能
	// 每一步改动：1. 更改标题为"客户端用户选择"；2. 使用clientUserApi.clientUserSelector作为分页API；3. 假设使用相同的orgTreeApi，如果需要client特定树，请进一步调整。
	import { message } from 'ant-design-vue'
	import { remove, isEmpty, cloneDeep } from 'lodash-es'
	import clientUserApi from '@/api/client/clientUserApi' // 修改为clientUserApi
	// 弹窗是否打开
	...
	const props = defineProps({
		...
		userPageApi: {
			type: Function,
			default: () => clientUserApi.clientUserSelector // 默认使用clientUserSelector
		},
		userListByIdListApi: {
			type: Function
		},
	...
	const userListByIdList = (param) => {
		if (typeof props.userListByIdListApi === 'function') {
			...
			return clientUserApi.userListByIds(param) // 修改为使用userListByIds以支持idList数组参数
		}
	}
	// 打开弹框
	const showClientUserModal = (ids = []) => {
