...
<script setup name="uploadIndex">
	...
	const props = defineProps({
		...
		// 上传并返回path
		uploadDynamicReturnPathApi: {
			type: String,
			default: convertUrl('/dev/file/uploadDynamicReturnPath'),
			required: false
		},
	...
	const action =
		props.uploadResultType === 'id'
			...
			: props.uploadResultType === 'path'
				? sysConfig.API_URL + props.uploadDynamicReturnPathApi
				: sysConfig.API_URL + props.uploadDynamicReturnUrlApi

	// 构造文件对象
	const buildFileObject = (url, id, path) => {
		const data = url ? url : path ? path : sysConfig.API_URL + props.uploadIdDownloadUrl + id
		return {
			data: data,
			name: url ? url : path ? path : id,
			url: data,
			status: 'done',
			response: {
				data: url ? url : path ? path : id,
				code: 200
			}
		}
	}
	...
	const echo = (newVal) => {
		...
		if (props.uploadResultCategory === 'interval') {
			...
			// path隔离
			if (props.uploadResultType === 'path') {
				newVal.split(',').forEach((path) => {
					const file = buildFileObject(undefined, undefined, path)
					fileList.value.push(file)
					fileList.value.reverse()
				})
			}
		...
		if (props.uploadResultCategory === 'array') {
			if (props.completeResult) {
				...
				if (props.uploadResultType === 'path') {
					newVal.forEach((path) => {
						fileList.value.push(buildFileObject(undefined, undefined, path))
					})
				}
			}
		}
