		...
		<a-card :bordered="false" style="margin-bottom: 10px">
			<a-form ref="searchFormRef" name="advanced_search" class="ant-advanced-search-form" :model="searchFormState">
				<a-row :gutter="24">
					...
					<a-col :span="8">
						<a-form-item name="isAgent" label="代理商">
							<a-select v-model:value="searchFormState.isAgent" placeholder="请选择代理商">
			...
			>
				...
				<template #bodyCell="{ column, record }">
					...
					<template v-if="column.dataIndex === 'isAgent'">
						<a-switch
							:checked="record.isAgent === 'YES'"
							@change="onAgentChange(record)"
							checked-children="是"
...
<script setup name="clientUser">
	...
	// 设置代理
	const setAgent = (record) => {
		clientUserApi.userSetAgent(record).then(() => {
			tableRef.value.refresh()
		})
	}
	// 取消代理
	const cancelAgent = (record) => {
		clientUserApi.userCancelAgent(record).then(() => {
			tableRef.value.refresh()
		})
	}

	// 代理开关变化
	const onAgentChange = (record) => {
		if (record.isAgent === 'YES') {
			cancelAgent(record)
		} else {
			setAgent(record)
		}
